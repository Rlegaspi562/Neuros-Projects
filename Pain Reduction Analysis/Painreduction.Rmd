...
Changing Data Types
```{r}
#Turning inj_location into a factor with 2 levels
...
#Need to see if pain scores should be treated as factors, check email from prof
#can leave as numerics, depends on context though
...
Inj_data_plots_14_21 <- cleaned_data_14_21[-c(1:3, 15)] %>%
...
Inj_data_plots_19_21 <- cleaned_data_19_21[-c(1:3, 15)] %>%
  keep(is.numeric) %>%                       
  gather() %>%                                
  ggplot(aes(value)) +                       
    facet_wrap(~ key, scales = "free") +

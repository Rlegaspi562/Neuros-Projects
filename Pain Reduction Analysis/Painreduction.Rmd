...
cleaning_inj_data <- cleaning_inj_data %>% 
  mutate(inj_location = cleaning_inj_data$inj_location,
         ...
         site_num = cleaning_inj_data$site)
...
test_inj_eval_outcome <- cleaning_inj_data %>% 
  select(subj_id, injection_evaluation_outcome) %>% 
...
cleaning_inj_data2 <- test_inj_eval_outcome %>% mutate(pass = test_inj_eval_outcome$pass) 
```
...
Inj_data_plots_19_21 <- cleaned_data_19_21[-c(1:3, 15,22)] %>%
    keep(is.numeric) %>%                       # Using numeric columns
  gather() %>%  
  ggplot(aes(value)) +                       #plotting
...
```{r}
ggplot(cleaned_data) +
  aes(x = site_num, fill = lido_eval_outcome) +
  geom_bar() +
...
## Who passed by what injection location?

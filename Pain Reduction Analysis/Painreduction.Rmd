...
cleaned_data <- cleaning_inj_data %>%  mutate(visit_dates = visit_date1) 
...
```{r}
 test2 <-  cleaned_data$inj_location %>% mutate(gsub(pattern = '[0-9]+', replacement =  '', cleaned_data)) %>% glimpse()

?gsub()

?str_replace_all()
str_replace_all(c('181173 - Bifurcation of Tibial and Common Peroneal nerves',
                  '180173 - Sciatic Nerve'), 
                "[:digit:]", "")
```
...
```{r}
colnames(new_cleaned_inj_data)
Inj_data_plots_14_21 <- new_cleaned_inj_data[-c(1:3, 14)] %>%
  keep(is.numeric) %>%                       # Using numeric columns
  gather() %>%  
  ggplot(aes(value)) +                       #plotting
    facet_wrap(~ key, scales = "free") +
    geom_histogram()                        #try geom_density/histo
...
Inj_data_plots_19_21 <- cleaned_data %>%
  keep(is.numeric) %>%                       
  gather() %>%                                
  ggplot(aes(value)) +                       
    facet_wrap(~ key, scales = "free") +
...
num_inj_data <- new_cleaned_inj_data %>%

...
cleaning_inj_data$inj_location <- str_replace_all(cleaned_data$inj_location, 
              "[:digit:]|[:punct:]", 
              "") %>% 
              str_trim()

#turning into cleaned_data
...
cleaning_inj_data$inj_location
...
Inj_data_plots_14_21 <- cleaned_data_14_21[-c(1:3, 14)] %>%
...
Inj_data_plots_19_21 <- cleaned_data_19_21[-c(1:3, 14)] %>%
  keep(is.numeric) %>%                       
  gather() %>%                                
  ggplot(aes(value)) +                       
    facet_wrap(~ key, scales = "free") +
